...
import { captureKnownAgents, summarizeSocialInteractions, SocialSummary } from './SocialMetrics';
...
import { AgentEngine, isWorkingHour } from '../agents/AgentEngine';
import { ActionExecutor } from './ActionExecutor';
import { RelationshipManager } from '../relationships/RelationshipManager';
import { MemoryManager } from '../memory/MemoryManager';
import { EventManager } from '../events/EventManager';
import { WorldEventType } from '../events/WorldEventType';
...
export interface SimulationSummary {
  ...
  nightEventsCount: number;
  nightMovementsCount: number;
  nightSocializationsCount: number;
  immediateBreakfastCount: number;
  agentSummaries: AgentSummary[];
  warnings: string[];
  successes: string[];
}

export interface WorkAgentSummary {
  name: string;
  firstArrival: string | null;
  lastWork: string | null;
  ticksAtWork: number;
  workTicks: number;
  eatInterruptions: number;
  restInterruptions: number;
  workdayPresencePercent: number;
}
...
export interface WorkSummary {
  ...
  outsideHoursActions: number;
  socializationsDuringWork: number;
  agents: WorkAgentSummary[];
}
...
export function runDaySimulation(): SimulationSummary {
  ...
  const agents: Agent[] = [
    new Agent('agent-ana', 'Ana', personalities.Ana,
      { energy: 90, hunger: 25, socialNeed: 50, mood: 75, isSleeping: false }, [], 'loc-casa'),
    new Agent('agent-carlos', 'Carlos', personalities.Carlos,
      { energy: 70, hunger: 40, socialNeed: 30, mood: 65, isSleeping: false }, [], 'loc-trabajo'),
    new Agent('agent-sofia', 'Sofía', personalities.Sofia,
      { energy: 85, hunger: 15, socialNeed: 65, mood: 80, isSleeping: false }, [], 'loc-parque'),
    new Agent('agent-diego', 'Diego', personalities.Diego,
      { energy: 60, hunger: 60, socialNeed: 20, mood: 55, isSleeping: false }, [], 'loc-cafe'),
    new Agent('agent-mariana', 'Mariana', personalities.Mariana,
      { energy: 75, hunger: 35, socialNeed: 45, mood: 70, isSleeping: false }, [], 'loc-casa'),
  ];

  // ─── 3. Inicialización del entorno y motores ──────────────────────────────
  const world = new World('IVANIA City', locations, agents);
  const initialKnownAgents = captureKnownAgents(world.agents);
  const clock = new SimulationClock(1, 0, 0); // Día 1 - 00:00
  const agentEngine = new AgentEngine(world);
  const relationshipManager = new RelationshipManager();
  const memoryManager = new MemoryManager();
  const eventManager = new EventManager(world, clock);
  ...
  const sleepingTicksByAgent: Record<string, number> = {
    'agent-ana': 0,
    'agent-carlos': 0,
    'agent-sofia': 0,
    'agent-diego': 0,
  ...
  const energyAtWakeup: Record<string, number> = {};
  const firstMealAfter0700: Record<string, string> = {};

  const workAgents: WorkAgentSummary[] = agents.map(agent => ({
  ...
  for (let tickIndex = 1; tickIndex <= TOTAL_TICKS; tickIndex++) {
    ...
    for (const [agentId, result] of tickResults) {
      if (!result.action.success) failedActions.push({
    ...
    const inWorkday = isWorkingHour(currentHour);
    if (inWorkday) scheduledTicks++;
    for (const [index, agent] of agents.entries()) {
      const action = tickResults.get(agent.id)?.action;
      const metrics = workAgents[index];
      if (!action?.success) continue;
      if (action.intent === 'WORK' && !inWorkday) outsideHoursActions++;
      if (!inWorkday) continue;
      ...
      if (action.intent === 'EAT') metrics.eatInterruptions++;
      if (action.intent === 'REST') metrics.restInterruptions++;
    }
  ...
  const social = summarizeSocialInteractions(world.agents, allEvents, initialKnownAgents);
  ...
  const work: WorkSummary = {
    ...
    outsideHoursEvents: allEvents.filter(e => e.type === 'AGENT_WORKED' && !isWorkingHour(e.hour)).length,
    outsideHoursActions,
    socializationsDuringWork: allEvents.filter(e => e.type === 'AGENTS_SOCIALIZED' && isWorkingHour(e.hour)).length,
    agents: workAgents,
  };
  console.log('WORK_METRICS ' + JSON.stringify(work));

  const isNightEvent = (e: { hour: number }) => e.hour >= 23 || e.hour < 7;
  ...
  const immediateBreakfastMeals = allEvents.filter(
    (e) => e.type === 'AGENT_ATE' && e.hour === 7 && e.minute <= 30
  );
