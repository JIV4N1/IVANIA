...
export function runIsolatedBiologicalTests(): void {
  ...
  const world1 = new World('World Bio 1', locations);
  const clock1 = new SimulationClock(1, 10, 0); // 10:00 AM
  const agent1 = new Agent('agent-1', 'Ana', defaultPersonality,
    { energy: 80, hunger: 20, socialNeed: 20, mood: 70, isSleeping: false }, [], 'loc-cafe');
  world1.addAgent(agent1);

  const eventManager1 = new EventManager(world1, clock1);
  const engine1 = new SimulationEngine(
    world1, clock1, new AgentEngine(world1),
    new ActionExecutor(world1, new RelationshipManager(), new MemoryManager(), eventManager1, clock1),
    eventManager1
  );
  ...
  assert(Math.abs(hungerDeltaDay - 9) < 0.001 && Math.abs(energyDeltaDay - 6) < 0.001, "Incrementos diurnos correctos");
  ...
  const world2 = new World('World Bio 2', locations);
  const clock2 = new SimulationClock(1, 1, 0); // 01:00 AM (noche)
  const agent2 = new Agent('agent-2', 'Carlos', defaultPersonality,
    { energy: 30, hunger: 20, socialNeed: 20, mood: 70, isSleeping: true }, [], 'loc-casa');
  world2.addAgent(agent2);

  const eventManager2 = new EventManager(world2, clock2);
  const engine2 = new SimulationEngine(
    world2, clock2, new AgentEngine(world2),
    new ActionExecutor(world2, new RelationshipManager(), new MemoryManager(), eventManager2, clock2),
    eventManager2
  );
  ...
  assert(Math.abs(hungerDeltaSleep - 2.4) < 0.001 && Math.abs(energyDeltaSleep - 18) < 0.001 && Math.abs(socialDeltaSleep - 1.8) < 0.001, "Deltas de sueño correctos");
  ...
  const world3 = new World('World Bio 3', locations);
  const clock3 = new SimulationClock(1, 12, 0);
  const agent3 = new Agent('agent-3', 'Sofía', defaultPersonality,
    { energy: 80, hunger: 85, socialNeed: 20, mood: 70, isSleeping: false }, [], 'loc-cafe');
  world3.addAgent(agent3);

  const eventManager3 = new EventManager(world3, clock3);
  const executor3 = new ActionExecutor(
    world3, new RelationshipManager(), new MemoryManager(), eventManager3, clock3
  );
  ...
  assert(agent3.state.hunger === 30, "Hambre quedó exactamente en 30");
  ...
  const agentClamp = new Agent('agent-clamp', 'Diego', defaultPersonality,
    { energy: 95, hunger: 20, socialNeed: 20, mood: 70, isSleeping: false }, [], 'loc-cafe');
  ...
  assert(agentClamp.state.hunger === 0, "Límite inferior respetado");
