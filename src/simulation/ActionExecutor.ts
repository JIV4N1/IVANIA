...
export class ActionExecutor {
   ...
   */
  execute(agent: Agent, decision: AgentDecision): ActionResult {
  ...
  private executeEat(agent: Agent): ActionResult {
    const before = agent.state.hunger;
    agent.state = this.withState(agent.state, {
      hunger: clamp(agent.state.hunger - EAT_HUNGER_REDUCTION, 0, 100),
    });
  ...
  private executeRest(agent: Agent): ActionResult {
    const before = agent.state.energy;
    agent.state = this.withState(agent.state, {
      energy: clamp(agent.state.energy + REST_ENERGY_RECOVERY, 0, 100),
    });
  ...
  private executeWork(agent: Agent): ActionResult {
    const beforeEnergy = agent.state.energy;
    const beforeHunger = agent.state.hunger;

    agent.state = this.withState(agent.state, {
      energy: clamp(agent.state.energy - 1, 0, 100),
      hunger: clamp(agent.state.hunger + 0.5, 0, 100),
    });
  ...
  private executeSocialize(agent: Agent, decision: AgentDecision): ActionResult {
    ...
    const initAfter = clamp(initBefore - SOCIALIZE_INITIATOR_REDUCTION, 0, 100);
    const targetAfter = clamp(targetBefore - SOCIALIZE_TARGET_REDUCTION, 0, 100);
  ...
  private executeIdle(agent: Agent): ActionResult {
    return {
      agentId:     agent.id,
      intent:      'IDLE',
      success:     true,
  ...
  private executeMove(agent: Agent, decision: AgentDecision): ActionResult {
   ...
   */
  private withState(state: AgentState, patch: Partial<AgentState>): AgentState {
    return { ...state, ...patch };
  }
}
