...
class AgentEngine {
    ...
    decide(agent, hour) {
        ...
        if (hunger >= HUNGER_THRESHOLD) {
            return this.resolveNeed(agent, 'EAT', hunger, `Hunger is above ${HUNGER_THRESHOLD} (${hunger})`);
        }
        if (energy <= ENERGY_THRESHOLD) {
            return this.resolveNeed(agent, 'REST', energy, `Energy is below or equal to ${ENERGY_THRESHOLD} (${energy})`);
        }
        ...
        if (socialNeed >= socialThreshold) {
            return this.resolveNeed(agent, 'SOCIALIZE', socialNeed, `Social need ${socialNeed} reached threshold ${socialThreshold} based on extroversion ${agent.personality.extroversion}`);
        }
     ...
     */
    getSocialThreshold(agent) {
        const rawThreshold = 90 - agent.personality.extroversion * 0.4;
        return (0, numberUtils_1.clamp)(rawThreshold, 50, 90);
    }
     ...
     */
    resolveNeed(agent, intent, _value, baseReason) {
        ...
        if (currentLocation && this.supportsCapability(currentLocation, capability)) {
            if (intent === 'SOCIALIZE') {
                ...
                for (const candidate of nearbyAgents) {
                    if (candidate.id === agent.id || candidate.state.isSleeping)
                        continue;
        ...
        const destination = this.world.locations.find((loc) => loc.id !== agent.locationId && this.supportsCapability(loc, capability));
        if (destination) {
